...
export function Sessions({ handleConversationCardClick }: SessionProps) {
    ...
    return (
      <div className="p-5">
        <Tooltip>
          ...
          <TooltipContent>{t('chat.conversations')}</TooltipContent>
        </Tooltip>
      </div>
    );
  }
  ...
  return (
    <aside
      className="p-5 w-[296px] flex flex-col bg-white border-r border-gray-200"
      role="complementary"
      data-testid="chat-detail-sessions"
    >
      <header className="flex items-center text-base justify-between gap-4 mb-4">
            ...
            <ul className="space-y-2">
              {conversationList.map((x) => (
                ...
                >
                  <button
                    type="button"
                    className="focus-visible:outline-none px-3 py-2 text-left flex-1 truncate hover:bg-gray-50 rounded-md"
