...
async def completion(tenant_id, agent_id, session_id=None, **kwargs):
    ...
    ref_chunks = (ref or {}).get("chunks", {})
    if ref_chunks:
        raw_lines = ["\n\n---\n**【知识库召回原文】**\n"]
        for i, (cid, ck) in enumerate(ref_chunks.items()):
            doc_name = ck.get("document_name", "")
            content = ck.get("content", "")
            similarity = ck.get("similarity")
            sim_str = f" (相似度: {similarity:.4f})" if similarity else ""
            raw_lines.append(f"**[{i+1}]** 来源: {doc_name}{sim_str}")
            raw_lines.append(f"> {content}\n")
        raw_block = "\n".join(raw_lines)
        txt += raw_block
        # 同步推一条 message 事件给流式客户端
        yield "data:" + json.dumps({
            "event": "message",
            "session_id": session_id,
            "data": {"content": raw_block, "role": "assistant"},
        }, ensure_ascii=False) + "\n\n"
